...
const path$c = require$$2;
const fs$j = require$$3;
...
const { Help: Help$1, stripColor } = help;
const { Option: Option$1, DualOptions: DualOptions2 } = option;
...
- either make a new Command for each call to parse, or stop storing options as properties`);
  ...
  _checkForMissingExecutable(executableFile, executableDir, subcommandName) {
    if (fs$j.existsSync(executableFile)) return;
 ...
 - ${executableDirMessage}`;
  ...
  _executeSubCommand(subcommand, args) {
    ...
    function findFile(baseDir, baseName) {
      ...
      if (sourceExt.includes(path$c.extname(baseName))) return void 0;
      const foundExt = sourceExt.find(
        (ext) => fs$j.existsSync(`${localBin}${ext}`)
      );
    ...
    if (this._scriptPath) {
      ...
      executableDir = path$c.resolve(
        path$c.dirname(resolvedScriptPath),
        executableDir
      );
    }
    if (executableDir) {
      ...
      if (!localFile && !subcommand._executableFile && this._scriptPath) {
        const legacyName = path$c.basename(
          this._scriptPath,
          path$c.extname(this._scriptPath)
        );
...
const { Argument: Argument2 } = argument;
const { Command: Command$1 } = command;
const { CommanderError: CommanderError2, InvalidArgumentError: InvalidArgumentError2 } = error;
const { Help: Help2 } = help;
const { Option: Option2 } = option;
commander.program = new Command$1();
commander.createCommand = (name) => new Command$1(name);
commander.createOption = (flags, description) => new Option2(flags, description);
commander.createArgument = (name, description) => new Argument2(name, description);
commander.Command = Command$1;
commander.Option = Option2;
commander.Argument = Argument2;
commander.Help = Help2;
commander.CommanderError = CommanderError2;
commander.InvalidArgumentError = InvalidArgumentError2;
commander.InvalidOptionArgumentError = InvalidArgumentError2;
const {
  Command: Command2
} = commander;
var fs$i = {};
...
var cwd = null;
var platform = define_process_env_default$1.GRACEFUL_FS_PLATFORM || process.platform;
...
if (define_process_env_default.TEST_GRACEFUL_FS_GLOBAL_PATCH && !fs$h.__patched) {
...
const u$e = universalify$2.fromPromise;
...
const u$d = universalify$2.fromPromise;
const fs$f = fs$i;
...
const fs$e = fs$i;
const u$c = universalify$2.fromPromise;
...
const u$b = universalify$2.fromPromise;
...
const u$8 = universalify$2.fromPromise;
const fs$9 = fs$i;
...
const u$7 = universalify$2.fromPromise;
...
const u$5 = universalify$2.fromPromise;
...
const fs$5 = fs$i;
const u$4 = universalify$2.fromPromise;
...
const u$3 = universalify$2.fromPromise;
...
const u$2 = universalify$2.fromPromise;
const fs$3 = fs$i;
...
const u = universalify$2.fromPromise;
var move = {
  move: u(move_1),
  moveSync: moveSync_1
};
...
program.name("webphone-sdk-cli").description("CLI tool for WebPhone SDK").version(pkg.version);
program.command("extract-sdk <targetPath>").description("Extract the WebPhone SDK to a specified path").action(async (targetPath) => {
  try {
    const sourcePath = require$$1$1.resolve(__dirname, "../index.umd.cjs");
    const destinationPath = require$$1$1.resolve(process.cwd(), targetPath);
    await fs.copy(sourcePath, destinationPath);
    console.log(`✅ SDK extracted to: ${destinationPath}`);
  } catch (error2) {
    console.error(`❌ Failed to extract SDK: ${error2.message}`);
  }
});
program.command("extract-schema <targetPath>").description("Extract the WebPhone SDK schema folder to a specified path").action(async (targetPath) => {
  try {
    const sourcePath = require$$1$1.resolve(__dirname, "../schema");
    const destinationPath = require$$1$1.resolve(process.cwd(), targetPath);
    await fs.copy(sourcePath, destinationPath);
    console.log(`✅ Schema folder extracted to: ${destinationPath}`);
  } catch (error2) {
    console.error(`❌ Failed to extract schema folder: ${error2.message}`);
  }
});
program.parse();
